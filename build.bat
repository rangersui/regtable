...
rem
rem   .\build            build the examples and test binaries
...
set LIB=src/regtable_core.c src/regtable_cli.c src/regtable_modbus.c src/regtable_mqtt.c
if "%1"=="strict" set CFLAGS=%CFLAGS% -Werror

if "%1"=="clean" (
    del /q example.exe regtest.exe mbtest.exe mqtest.exe tcpslave.exe mqttdemo.exe 2>nul
    exit /b 0
)
...
%CC% %CFLAGS% -o mqtest.exe     regtable_mqtt_test.c %LIB% || exit /b 1
%CC% %CFLAGS% -o tcpslave.exe   example_modbus_tcp.c   %LIB% -lws2_32 || exit /b 1
%CC% %CFLAGS% -o mqttdemo.exe   example_mqtt_desktop.c %LIB% || exit /b 1

if "%1"=="run"  .\example.exe
if "%1"=="test"   .\regtest.exe && .\mbtest.exe && .\mqtest.exe
if "%1"=="strict" .\regtest.exe && .\mbtest.exe && .\mqtest.exe
exit /b %ERRORLEVEL%
