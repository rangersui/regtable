...
(a typed Python client) into outdir/.
...
def py_symbol_problem(ident):
    ...
    if ident in PY_CLIENT_API:
        return "is a method of the generated Python client"
    return None
...
def emit_py(device, regs):
    ...
    L = []
    L.append(f'"""Generated by regtable_gen.py from the \'{device}\' description.')
    ...
    L.append("except ImportError:                  # not imported as a package member")
    L.append("    from regtable_client import RegtableClient, SerialTransport, PipeTransport, f32")
    L.append("")
    L.append("")
    ...
    for r in regs:
        ...
        if r["perm"] == "rw":
            ...
            if t == "bool":
                ...
                L.append("        try:")
                L.append("            v = f32(v)               # what the device will hold")
...
def main():
    ...
    (out / f"{device}_client.py").write_text(emit_py(device, regs), encoding="ascii")
    runtime = Path(__file__).resolve().parent / "regtable_client.py"
    (out / "regtable_client.py").write_text(runtime.read_text(encoding="utf-8"),
                                            encoding="utf-8")
    print(f"regtable_gen: {len(regs)} registers -> "
          f"{out / 'registers.c'}, registers.h, registers.md, {device}_client.py "
