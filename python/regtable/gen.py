...
def expand_svd(regs, base_dir):
    ...
    out = []
    cache = {}
    chip = {}                     # what the SVD header says the chip is
    for i, r in enumerate(regs):
        ...
        if full not in cache:
            try:
                ...
                fail(f"{where}: {e}")
        table = cache[full]
        # several SVD files in one YAML describe one chip: every header
        # fact they both state must agree, and a fact only one states
        # is kept; order does not matter
        this = getattr(table, "chip", {})
        if chip.get("name") and this.get("name") and chip["name"] != this["name"]:
            fail(f"{where}: svd files describe different chips: "
                 f"{chip['name']} and {this['name']}")
        for k, v in this.items():
            if k in chip and chip[k] != v:
                fail(f"{where}: svd files disagree on the chip {k}: "
                     f"{chip[k]!r} and {v!r}")
            chip[k] = v
        picks = r.get("pick")
        ...
        for j, item in enumerate(picks):
            ...
            entry = SvdEntry({
                ...
                "perm": "ro",
                "desc": desc,
                "_svd": {"file": path, "pick": pick,
                         "path": f"{reg.path}.{field.name}" if field else reg.path,
                         "addr": reg.addr, "size": reg.size,
...
def validate(doc, max_entries=64, base_dir=None):
    """Check the description and return (device, entries, chip): chip
    is what the SVD header says ({"name", "series", "cpu"}, empty with
    no svd: block). base_dir is where relative `svd:` paths resolve:
    the YAML's own directory."""
    ...
    located, chip = expand_svd(regs, base_dir)
...
def c_string(s):
    """s as a C string literal, quotes included: backslash and quote
    escaped, every ? escaped so no two ever form a trigraph. Every
    run-time string the emitters write into C goes through here."""
...
def emit_h(device, regs, chip=None):
    NREGS = len(regs)
    lines = [
        ...
        f'#define REGTABLE_GEN_{device.upper()}_NAME "{device}"',
    ]
    if chip and chip.get("name"):
        lines += ["", "/* the chip the SVD describes, for RegIdentity.chip */",
                  f'#define REGTABLE_GEN_{device.upper()}_CHIP {c_string(chip["name"])}']
        if chip.get("series"):
            lines.append(f'#define REGTABLE_GEN_{device.upper()}_SERIES {c_string(chip["series"])}')
        if chip.get("cpu") and svdmod.cpu_display(chip["cpu"]):    # "other" has no name to give
            lines.append(f'#define REGTABLE_GEN_{device.upper()}_CPU {c_string(svdmod.cpu_display(chip["cpu"]))}')
    lines += [
        "",
        "#ifdef __cplusplus",
        'extern "C" {',
...
def emit_c(device, regs):
    ...
    for r in regs:
        ...
        if r.get("desc"):
            f.append(f'.description = {c_string(r["desc"])}')
        body = ",\n      ".join(f)
        lines.append(f"    {{ {body} }},")
    lines += ["    { .name = NULL }", "};", ""]
    return "\n".join(lines)
...
def emit_py(device, regs):
    ...
    L.append("    }")
    L.append("    # registers read from the chip itself: name -> PERIPHERAL.REGISTER[.FIELD]")
    L.append("    # in the vendor's SVD; regtable fetch draws the peripherals on the pins")
    L.append("    __silicon__ = {")
    for r in regs:
        if "_svd" in r:
            L.append(f'        "{r["name"]}": {r["_svd"]["path"]!r},')
    L.append("    }")
    L.append("")
...
def build_client(yaml_file, max_entries=64):
    ...
    invalid description."""
    device, regs, _chip = validate(load(yaml_file), check_max_entries(max_entries),
...
def main(argv=None):
    ...
    try:
        device, regs, chip = validate(load(args.yaml_file),
    ...
    try:
        files = {
            "registers.h": emit_h(device, regs, chip).encode("ascii"),
